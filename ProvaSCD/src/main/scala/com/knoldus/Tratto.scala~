...
{	
  	...
  	override def receive: Actor.Receive = 
	{
		...
		case m:mezzoDeviato =>	gestisci (m);
		case m:Mezzo =>	gestisci (m); //Invia i mezzi perché il semaforo è verde
		case m:mezzoDeviatoPiuPriorita => gestisci (m.mezzo);
	...
	def redReceiver: Receive = 
	{
		...
		case m:mezzoDeviato => mezzi+=m; println("Mezzo (deviato) "+m.id+" in coda al tratto "+id);
		...
		case Verde =>	context.unbecome();
				while(!mezzi.isEmpty)
				{
					...
					if(!m.deviato)
						...
						self!new mezzoDeviatoPiuPriorita(m);
				}	   
	}
